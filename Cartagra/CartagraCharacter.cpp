...
{
	FVector Destination = (HitLocation - ActionInstigator->GetActorLocation()) * BaseStats.AttackRange;
...
void ACartagraCharacter::BeginPrimaryAttack(ACartagraCharacter* Target)
{
	...
	GetWorldTimerManager().SetTimer(AttackSpeedTimerHandle, this, &ACartagraCharacter::AttackSpeedTimer, BaseStats.AttackSpeed, false, BaseStats.AttackSpeed);
	bAttacking = true;
...
void ACartagraCharacter::SpawnProjectile(FVector Location, FRotator Rotation, ACartagraCharacter* ActionInstigator)
{
	...
	InterpComp->bIsDestroyable = true;
	InterpComp->Caller = ActionInstigator;
	InterpComp->OnHit.BindUFunction(this, FName("OnHit"));
	InterpComp->OnTargetHit.BindUObject(this, &ACartagraCharacter::OnTargetHit);
}

void ACartagraCharacter::CalculateDamage(ACartagraCharacter* TargetHit, struct FCharacterStats& AttackerStats)
{
	//After Calulating resistances, armor, procs...Apply the calculated Damage
	//for now this is not implemented

	int64 CalculatedDamage = AttackerStats.BaseDamage - TargetHit->BaseStats.BaseArmor;
...
{
	DamagedTarget->Health -= DamageToApply;
...
void ACartagraCharacter::OnTargetHit(ACartagraCharacter* Target, struct FCharacterStats& AttackerStats)
{
	//call CalculateDamage
	//perhaps add other Dynamic Delegates for procs..

	CalculateDamage(Target, AttackerStats);
}
